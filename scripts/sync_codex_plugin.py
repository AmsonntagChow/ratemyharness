...
SOURCE = ROOT / "skills" / "ratemyharness"
DESTINATION = ROOT / "plugins" / "ratemyharness" / "skills" / "ratemyharness"
...
def relative_files(root: Path) -> list[Path]:
    return sorted(
        ...
        if path.is_file()
        and not IGNORED_NAMES.intersection(path.relative_to(root).parts)
...
def sync() -> None:
    expected_parent = ROOT / "plugins" / "ratemyharness" / "skills"
    if DESTINATION.parent != expected_parent or DESTINATION.name != "ratemyharness":
