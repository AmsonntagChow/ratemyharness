...
SKILL_DIR = ROOT / "skills" / "ratemyharness"
SKILL_FILE = SKILL_DIR / "SKILL.md"
CODEX_PLUGIN_DIR = ROOT / "plugins" / "ratemyharness"
PACKAGED_SKILL_DIR = CODEX_PLUGIN_DIR / "skills" / "ratemyharness"
...
REQUIRED_REPO_FILES = [
    ...
    "plugins/ratemyharness/assets/logo.png",
    "plugins/ratemyharness/assets/logo.svg",
    "plugins/ratemyharness/skills/ratemyharness/SKILL.md",
    ".github/CODEOWNERS",
...
def validate_skill(errors: list[str]) -> None:
    ...
    elif "$ratemyharness" not in metadata_path.read_text(encoding="utf-8"):
        errors.append("agents/openai.yaml default prompt must mention $ratemyharness")
...
def validate_claude_plugin(errors: list[str]) -> None:
    ...
    if not isinstance(manifest, dict):
        return
    if manifest.get("name") != "ratemyharness":
        errors.append("Claude plugin manifest name must be 'ratemyharness'")
    ...
    if manifest.get("repository") != "https://github.com/AmsonntagChow/ratemyharness":
    ...
    if not isinstance(marketplace, dict):
        return
    if marketplace.get("name") != "amsonntagchow-ratemyharness":
        errors.append("Claude marketplace name must be 'amsonntagchow-ratemyharness'")
    plugins = marketplace.get("plugins")
    if not isinstance(plugins, list):
        errors.append("Claude marketplace plugins must be an array")
        return
    matches = [
        item for item in plugins if isinstance(item, dict) and item.get("name") == "ratemyharness"
    ]
    if len(matches) != 1:
        errors.append("Claude marketplace must contain exactly one ratemyharness plugin entry")
        return
    if matches[0].get("source") not in {".", "./"}:
        errors.append("Claude marketplace ratemyharness source must point to the repository root")
...
def validate_codex_plugin(errors: list[str]) -> None:
    manifest = load_json("plugins/ratemyharness/.codex-plugin/plugin.json", errors)
    if not isinstance(manifest, dict):
        return
    claude_manifest = load_json(".claude-plugin/plugin.json", errors)
    if manifest.get("name") != "ratemyharness":
        errors.append("Codex plugin manifest name must be 'ratemyharness'")
    ...
    if not isinstance(interface, dict):
        ...
        if interface.get("displayName") != "RateMyHarness":
            errors.append("Codex plugin displayName must be 'RateMyHarness'")
    ...
    if not isinstance(marketplace, dict):
        return
    if marketplace.get("name") != "ratemyharness":
        errors.append("Codex marketplace name must be 'ratemyharness'")
    marketplace_interface = marketplace.get("interface")
    if not isinstance(marketplace_interface, dict) or marketplace_interface.get("displayName") != "RateMyHarness":
        errors.append("Codex marketplace displayName must be 'RateMyHarness'")
    plugins = marketplace.get("plugins")
    if not isinstance(plugins, list):
        errors.append("Codex marketplace plugins must be an array")
    else:
        matches = [item for item in plugins if isinstance(item, dict) and item.get("name") == "ratemyharness"]
        if len(matches) != 1:
            errors.append("Codex marketplace must contain exactly one ratemyharness entry")
        else:
            source = matches[0].get("source")
            if source != {"source": "local", "path": "./plugins/ratemyharness"}:
                errors.append("Codex marketplace must point to ./plugins/ratemyharness")
            if matches[0].get("policy") != {
                "installation": "AVAILABLE",
                "authentication": "ON_INSTALL",
            }:
                errors.append("Codex marketplace policy must make the plugin available on install")
            if matches[0].get("category") != "Developer Tools":
    ...
    allowed_package_files.update(
        Path("skills/ratemyharness") / relative for relative in source_files
    )
    actual_package_files = set(directory_files(CODEX_PLUGIN_DIR))
    unexpected_package_files = sorted(actual_package_files - allowed_package_files)
    missing_package_files = sorted(allowed_package_files - actual_package_files)
    if unexpected_package_files:
        errors.append(f"skills-only package contains unexpected files: {unexpected_package_files}")
    if missing_package_files:
        errors.append(f"skills-only package is missing files: {missing_package_files}")
